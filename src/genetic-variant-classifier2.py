...
class Logger:
    """Thin shim around logging.getLogger so the rest of the code can stay tidy."""
...
class DataPreprocessor:
    ...
    def prepare_target(self,
                   df: pd.DataFrame,
                   *, binary_mode: bool = False) -> pd.DataFrame:
        """
        ➜ 3-class mode (benign / pathogenic / uncertain)      – default
        ➜ 2-class mode (pathogenic / non-pathogenic)          – binary_mode=True
        """
        self.logger.info(f"Preparing target variable (binary={binary_mode})")
        df = df.copy()

        # 1️⃣ raw text → canonical label
        def map_to_label(txt):
            if pd.isna(txt):
                return None
            t = str(txt).lower()
            if "pathogenic" in t:
            ...
            if "benign" in t and "pathogenic" not in t:
                return "benign"
            if "uncertain" in t or "conflicting" in t:
                return "uncertain"
            return None

        df["mapped_target"] = df["clinical_significance"].apply(map_to_label)
        df.dropna(subset=["mapped_target"], inplace=True)

        # 2️⃣ optionally collapse to binary
        if binary_mode:
            df["mapped_target"] = df["mapped_target"].replace(
                {"benign": "non_pathogenic", "uncertain": "non_pathogenic"})
            label_map = {"non_pathogenic": 0, "pathogenic": 1}
        else:
            label_map = {"benign": 0, "pathogenic": 1, "uncertain": 2}

        df["target_encoded"] = df["mapped_target"].map(label_map)

        # 3️⃣ distribution sanity-check
        dist = df["mapped_target"].value_counts()
        self.logger.info("Target class distribution:\n%s", dist.to_string())

        plt.figure(figsize=(6, 4))
        dist.plot(kind="bar", color="#2ecc71")
...
class GeneticVariantAnalysis:
    ...
    # genetic-variant-classifier2.py  ▸ class GeneticVariantAnalysis
    def run_exploratory_analysis(self, *, binary_mode: bool = False) -> pd.DataFrame:
        """
        EDA + first-pass feature prep.

        Parameters
        ----------
        binary_mode : bool
            True  ⇒ pathogenic vs non-pathogenic (2-class)
            False ⇒ benign / pathogenic / uncertain (3-class)
        """
        df = self.data_loader.load_data()

        # *** single call, forward the flag ***
        df = self.preprocessor.prepare_target(df, binary_mode=binary_mode)
...
# ------------------------------------------------------------------------
    ...
    def run_holdout_evaluation(self,
        ...
        if multiclass:
            self.logger.info("Detected multiclass target → fairness skipped")
        ...
        for mode, (Xtr, Xte, popte) in [
            ...
            ax.set_title(f"{model_name} – {mode.replace('_', ' ')}")
            out_png = PLOTS_DIR / f"{model_name}_{mode}_holdout.png"
            fig.tight_layout()
...
# MAIN PIPELINE
# ---------------------------------------------------------------------------
def main(*, binary_mode: bool = False) -> None:
    """
    ...
    try:
        ...
        df_pop = analysis.run_exploratory_analysis(binary_mode=binary_mode)

        # ------------------------------------------------------------------ #
        # 2. Cross-validation
        # ------------------------------------------------------------------ #
        logger.info("\n=== Running Cross-Validation ===")
        analysis.run_cross_validation(df_pop)
        ...
        # ------------------------------------------------------------------ #
        # 4. Hyper-parameter tuning (checkpointed, resumes if done before)
        # ------------------------------------------------------------------ #
        ...
        # ------------------------------------------------------------------ #
        # 5. Hold-out evaluation  (loop over the three model families)
        # ------------------------------------------------------------------ #
...
# ---------------------------------------------------------------------------
# CLI WRAPPER   →  call   python genetic-variant-classifier2.py  [--binary]
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    import argparse, pathlib, joblib
    parser = argparse.ArgumentParser(description="Genetic-variant ML pipeline")
    parser.add_argument(
        "--binary",
        action="store_true",
        help="train binary (pathogenic vs non-pathogenic) model"
    )
    args = parser.parse_args()
    main(binary_mode=args.binary)
